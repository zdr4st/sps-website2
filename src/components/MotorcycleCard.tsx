...
export function MotorcycleCard({ motorcycle }: MotorcycleCardProps) {
  return (
    <Card className="group flex flex-col overflow-hidden border-gray-200 shadow-none hover:shadow-xl transition-all duration-300 rounded-2xl bg-white">
      ...
      <CardHeader className="p-5 pb-2 border-t border-gray-100">
      ...
      <CardFooter className="p-5 flex gap-3 pt-0">
        <Link href={`/motorcycle/${motorcycle.id}`} className="flex-1">
          <Button variant="outline" className="w-full rounded-xl border-gray-300 text-gray-700 font-bold hover:bg-gray-50 hover:text-primary transition-colors">
            Detail
          </Button>
        </Link>
        <Link 
          ...
          className="flex-1"
        >
          <Button className="w-full rounded-xl font-bold bg-primary hover:bg-primary/90 text-white shadow-md">
            Hubungi
          </Button>
        </Link>
      </CardFooter>
